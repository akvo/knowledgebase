...
Akvo Lumen supports you in understanding your data. But the data journeys starts by designing what you are interested in learning - collecting data for - and then you get the data. Akvo Flow supports you in these initial stages of the data journey and it is where you define your monitoring. 



Design monitoring survey 
...
Leave the data type for ‘Submitted at’ set as ‘date’ data type.
Under ‘+Transform’ select ‘Derive column’.
Set the new column title as ‘Month’.
Set the new column type as text.
Decide what to do if the transformation fails. We suggest to ‘leave cells empty’ as you can quickly evaluate if everything went well.
Use this formula for the javascript code: ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"][new Date(Number(row['Submitted at'])).getMonth()]
...
Under '+Transform' select Derive Category column. 
Select the Month column. 
Create new categories for each month, where January = 01, February =02, etc. 
Derive the new column. 
Now you have a column holding number representation of months. This column is still of the data type TEXT. 
Click on the datatype and change it to Number. 
Now your column is ready to be used in a line chart, in the x axis where each month is represented. 
...
Change the data type for ‘Submitted at’ from ‘date’ to ‘text’.
Under ‘+Transform’ select ‘Split column values’.
Select ‘Submitted at’ in the column dropdown.
As we only want to work with ‘2019-09-15’ we need to split the values by ‘space’. In the‘Separator’ text field add a space (empty space).
Set the ‘New column prefix’ to for example Date
Hit Extract.
...
In the Monitoring form dataset note down the ID of the data point you want to analyse. You will find it under the column ‘Identifier’. Simply highlight the value in the cell and copy it.
Now open a new visualisation. To set up any chart in Lumen, you can go directly from the monitoring dataset to the visualisations editor, or open the visualisations editor and select chart and your dataset.
Under the Dataset open ‘Advanced’. Here you can add filters to the visualisation. Click on ‘New filter’.
Under ‘Filter operation’ select ‘Keep rows in which’. This means the visualisation will only be based on the rows meeting the condition you set next.
For ‘Column to filter by’ select ‘Identifier’.
For ‘ Filter match method’ select ‘exactly matches’
Add the data point ID you copied to the last box, ‘Filter match value’.
Save the filter.
