...
We are currently working on a data visualisation tool, Akvo Lumen, that is directly linked to Flow and allows you to create advance data visualisations. 
...
Select the folder, survey and form from the top dropdown menus.
Select the export option that fits your needs the best. 
For some export options you can limit the dataset by setting the dates when the data was submitted. 
Hit Generate. 
...
## NEW - 10 April 2019 - Form version in data exports
...
Answers to option questions where you could select more than one answer are combined in one cell 
Answers to different levels in cascade questions are combined in one cell.    
...
data cleaning export with RQGs
...
Open your terminal (you can use cmd+space and search for "terminal")
Paste the following Url: /usr/bin/ruby -e "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/master/install)" . Press the return key and enter your password.
If you run into problems, paste "brew doctor" and follow the described steps.
Install wget by pasting: brew install wget.
Downloading the Photos

Download the RAW Data Report from the Akvo Flow Dashboard
Copy the column containing the photo links and paste it into a new notepad or Text edit. 
Save the new notepad as a txt file. e.g photos.txt
Create a folder on your desktop called photos.
Copy the saved txt file, photos.txt to the folder photos
Open a new terminal window
Paste the following command:   cd ~/desktop/photos
Afterwards, again paste the command wget -i photos.txt
The photos have been downloaded to the  "photos" folder.
...
Insert one extra column on the side of your target column.
Select the column with the coded option questions inside.
Press data followed by text to column.
The file type that describes our data best is delimited as the ":" appears in every cell.
In the next screen we set the delimiter to other with the value ":"
...
Akvo Lumen makes it possible to connect your Flow data from a monitoring survey using the Merge functionality. Here you can see more on how this Lumen feature works.
...
# Importing data
## Data cleaning and import with repeated question groups
...
Copy in the answers to the raw data sheet (1st sheet)
Create the IDs using the pattern "new-1"
Add the metadata to the rows 
Add answers to the repeated question group sheet one submission at a time. 
Add the right ID to the rows 
Add the number of repetitions in the Repeat no column 
Repeat for the next submission.  
